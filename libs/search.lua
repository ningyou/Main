...
function _M:moviedb(search)
	...
	if search:match("tt(%d+)") then
		local movie, status = http:get("http://api.themoviedb.org/2.1/Movie.imdbLookup/en/json/" .. apikey"moviedb" .. "/" .. search)
		...
		local movie, status = http:get("http://api.themoviedb.org/2.1/Movie.search/en/json/" .. apikey"moviedb" .. "/" .. search)
		if movie and status == 200 then
			movie = json.decode(movie)
			return movie
		end
